...
- **`relay_down`** → no relay is listening. This is the usual cause of
  "connecting…". Start the persistent relay (`references/setup.md`, Path A
...
- Path A: JSON results on stdout from the CLI (parse before reasoning). Screenshots under `~/.browser-hand/screenshots/`.
...
- **Relying on the per-call ephemeral relay for the extension connection.** A relay that lives only for one CLI invocation races the extension's reconnect interval and usually loses. Keep a persistent relay up (`references/setup.md`, Path A step 2).
...
- **Reaching for bare `dev-browser run` (no `--connect`) to grab a screenshot.** On this machine the daemon's bundled Chromium SIGABRTs on launch — you get a crash, not a page. For anonymous captures use the throwaway-Chrome escape hatch (`references/path-b-headless-connect.md`); for authenticated ones use Path A/B against the user's real Chrome.
